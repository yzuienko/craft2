(function () {
  ...
  function renderCraftCard(view, meta) {
    ...
    const crystalsHint = `${formatMoney(current.output.baseCrystals)} базовых × ${formatDecimal(current.output.multiplier)}`;
      ...
      <article class="craft-card ${meta.isBest ? "is-top" : ""}">
        ...
        <div>
          ...
          <div class="metrics-grid">
            ...
            ${renderMetric(`Кристаллов ×${formatDecimal(current.output.multiplier)}`, `${formatDecimal(current.output.totalExpected)} шт.`, crystalsHint)}
          </div>
        </div>
